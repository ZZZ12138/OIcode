...
int main() {
  ...
  /*	for (int i = n-1; i >= n-pre; i--){
                  if(sort_mark[i] == sort_mark[i-1])
     continue;//遇到同分数的找最后一个 int count_the_same_mark = 0; for (int j =
     0; i < n; j++){ if(person[1][j] == sort_mark[i]) {
                                          out_person[0][count_person] =
     person[0][j]; out_person[1][count_person] = person[1][j]; min_mark =
